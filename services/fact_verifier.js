...
class FactVerifier {
  ...
  verifyAll(newsItems = [], logger = console.log) {
    ...
    for (const item of newsItems) {
      ...
      const containsBlacklist = this.blacklist.some((word) =>
        titleLower.includes(word),
      );
      if (containsBlacklist) {
        logger(
          `[Fact Verification] [Rejected] "${item.title}" contains unverified/blacklisted terminology.`,
        );
        continue;
      }

      // Rule 2: Cross-verification for RSS feeds (at least minSources required)
      ...
      const confidence =
        sourcesCount >= this.minSources ? 100 : sourcesCount === 2 ? 98 : 95;
      ...
      if (confidence >= this.threshold) {
        ...
        logger(
          `[Fact Verification] [Rejected] "${item.title}" - Confidence: ${confidence}% (${sourcesCount} sources matching).`,
        );
      }
    }

    logger(
      `[Fact Verification] Fact check completed. ${verifiedItems.length} of ${newsItems.length} stories verified.`,
    );
