...
const isQueuePage = pageType === "queue";
...
function initWebSocket() {
  const stateBadge = getEl("ws-state");
...
function appendTerminalLog(log) {
  const logViewer = getEl("terminal-log-viewer");
  if (!logViewer) return;
...
async function updateDashboardData() {
  ...
  if (isQueuePage) {
    await Promise.all([fetchSystemStatus(), fetchQueue()]);
    return;
  }
...
async function fetchSystemStatus() {
  try {
    ...
    if (modeBadge) {
      modeBadge.textContent = status.simulationMode
        ? "SIMULATION"
        : "LIVE MODE";
      modeBadge.className = status.simulationMode
        ? "status-value sim-badge"
        : "status-value active-badge";
    }

    const stateBadge = getEl("pipeline-state");
    if (stateBadge) {
      stateBadge.textContent = status.activePipelineRun ? "RUNNING" : "IDLE";
      stateBadge.className = status.activePipelineRun
        ? "status-value active-badge fa-spin"
        : "status-value connect-badge";
    }

    setText("landing-queue-count", status.pendingQueueCount ?? "0");
    setText("landing-next-publish", status.nextPublishTime || "--");
  } catch (e) {
...
async function fetchNewsFeed() {
  try {
    ...
    const countBadge = getEl("news-count");
    if (countBadge)
      countBadge.textContent = `${allPosts.length} posts generated`;
    if (getEl("landing-post-count"))
      setText("landing-post-count", allPosts.length);

    const newsList = getEl("news-list");
    if (!newsList) return;
    ...
    const selectedCategory =
      document.querySelector(".filter-tab.active").dataset.category;
    let html = "";
    ...
    newsList.innerHTML =
      html ||
      `<div class="loading-placeholder">No stories in this category.</div>`;
  } catch (e) {
...
async function fetchQueue() {
  try {
    ...
    queue.forEach((item) => {
      ...
      const timeStr = pubDate.toLocaleTimeString([], {
        hour: "2-digit",
        minute: "2-digit",
      });
    ...
    if (getEl("queue-count")) {
      setText("queue-count", queue.length);
    }

    if (getEl("home-queue-summary")) {
...
async function publishPostDirectly(id) {
  try {
    appendTerminalLog({
      timestamp: new Date(),
      type: "info",
      message: `Manual publish override triggered for: ${id}`,
    });
...
async function fetchAnalytics() {
  try {
    ...
    const reachElem = getEl("metric-reach");
    if (reachElem)
      reachElem.textContent = Number(data.summary.total_reach).toLocaleString();
    const erElem = getEl("metric-er");
    if (erElem) erElem.textContent = `${data.summary.average_engagement_rate}%`;
    const savesElem = getEl("metric-saves");
    if (savesElem)
      savesElem.textContent = Number(data.summary.total_saves).toLocaleString();
    const sharesElem = getEl("metric-shares");
    if (sharesElem)
      sharesElem.textContent = Number(
        data.summary.total_shares,
      ).toLocaleString();
    ...
    const chartCanvas = getEl("analytics-chart");
    if (chartCanvas) {
    ...
    if (getEl("landing-reach")) {
      setText(
        "landing-reach",
        Number(data.summary.total_reach).toLocaleString(),
      );
    }
    if (getEl("home-analytics-summary")) {
      setText(
        "home-analytics-summary",
        `${Number(data.summary.total_reach).toLocaleString()} reach · ${data.summary.average_engagement_rate}% ER`,
      );
    }
...
async function fetchRecommendations() {
  try {
    ...
    const container = getEl("recommendations-container");
    if (!container) return;
    ...
    recs.forEach((rec) => {
      const icon =
        rec.type === "BOOST"
          ...
          : rec.type === "REDUCE"
            ? `<i class="fa-solid fa-circle-arrow-down text-info"></i>`
    ...
    container.innerHTML =
      html || `<div class="rec-item">No active recommendations.</div>`;
  } catch (e) {
...
async function savePostEdits() {
  ...
  try {
    ...
    appendTerminalLog({
      timestamp: new Date(),
      type: "success",
      message: `Edits saved for "${title}" successfully.`,
    });
  } catch (e) {
    appendTerminalLog({
      timestamp: new Date(),
      type: "error",
      message: `Failed to save edits: ${e.message}`,
    });
  } finally {
...
async function triggerPipelineRun() {
  ...
  try {
    appendTerminalLog({
      timestamp: new Date(),
      type: "info",
      message: "Manual pipeline run request dispatched...",
    });
...
async function triggerAnalyticsRefresh() {
  ...
  try {
    appendTerminalLog({
      timestamp: new Date(),
      type: "info",
      message: "Refreshing performance stats...",
    });
    ...
    appendTerminalLog({
      timestamp: new Date(),
      type: "error",
      message: `Stats refresh failed: ${e.message}`,
    });
  } finally {
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (isLandingPage) {
    ...
    const svgTab = getEl("view-svg-tab");
    const pngTab = getEl("view-png-tab");
  ...
  addListener("btn-run-pipeline", "click", triggerPipelineRun);
  addListener("btn-refresh-analytics", "click", triggerAnalyticsRefresh);
  addListener("btn-save-post", "click", savePostEdits);
  addListener("btn-clear-logs", "click", () => {
    const terminalLogViewer = getEl("terminal-log-viewer");
    if (terminalLogViewer) terminalLogViewer.innerHTML = "";
    appendTerminalLog({
      timestamp: new Date(),
      type: "system",
  ...
  const publishQueueBtn = getEl("btn-publish-queue");
  if (publishQueueBtn) {
    publishQueueBtn.addEventListener("click", async () => {
      try {
